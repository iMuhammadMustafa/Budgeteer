...
import { RECURRING_CONSTANTS } from "@/src/types/enums/recurring";
...
export function calculateNextOccurrence(currentDate: Date, intervalMonths: number): Date {
  ...
  if (intervalMonths < RECURRING_CONSTANTS.INTERVAL_MONTHS.MIN || 
    ...
    throw new Error(`Interval months must be between ${RECURRING_CONSTANTS.INTERVAL_MONTHS.MIN} and ${RECURRING_CONSTANTS.INTERVAL_MONTHS.MAX}`);
  }
...
export function getIntervalOption(intervalMonths: number) {
  return COMMON_INTERVALS.find(option => option.months === intervalMonths) || {
