...
describe("Credit Card Statement Date Functionality", () => {
  ...
  beforeEach(() => {
    creditCardService = new CreditCardPaymentService(mockAccountRepo, mockTransactionRepo, mockRecurringRepo);
    jest.clearAllMocks();
  });
  ...
  describe("Statement Balance Calculation", () => {
    ...
    it("should handle statement date ranges correctly for current month", async () => {
      ...
      const calculateStatementBalanceByDate = (creditCardService as any).calculateStatementBalanceByDate.bind(
        creditCardService,
      );
  ...
  describe("Credit Card Payment Creation", () => {
    it("should create credit card payment with statement date validation", async () => {
      ...
      mockAccountRepo.findById.mockResolvedValueOnce(mockSourceAccount).mockResolvedValueOnce(mockLiabilityAccount);
      ...
      const result = await creditCardService.createCreditCardPayment(mockRequest, "tenant-1", "user-1");
