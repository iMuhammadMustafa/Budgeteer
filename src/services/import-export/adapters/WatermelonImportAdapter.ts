...
/**
 * WatermelonDB Import Adapter
 * Implements import functionality for WatermelonDB storage mode (Local/Demo)
 */
export class WatermelonImportAdapter implements IImportAdapter {
  ...
  async isReady(): Promise<boolean> {
    try {
      ...
      console.error("WatermelonImportAdapter: Not ready", error);
      return false;
    }
  }
  ...
  async checkExistingRecords(tableName: TableNames, ids: string[], tenantId: string): Promise<Set<string>> {
    ...
    try {
      ...
      ids.forEach(id => {
        const exists = allRecords.some((record: any) => {
          const recordId = record.id || record._raw?.id;
  ...
  async getDependencyData(tableName: TableNames, tenantId: string): Promise<Map<TableNames, Set<string>>> {
    ...
    for (const depTableName of config.dependencies) {
      try {
        ...
        dependencyData.set(depTableName, plainRecords);
  ...
  async importTable(
    ...
    try {
      ...
      if (options.skipDuplicates) {
        const repository = this.getRepositoryForTable(tableName);
        const watermelonRecords = await repository.findAll(tenantId);
        existingRecords = watermelonRecords.map((record: any) => this.convertWatermelonRecord(record));
      }
      ...
      for (let i = 0; i < records.length; i += batchSize) {
        ...
        for (let j = 0; j < batch.length; j++) {
          ...
          try {
            ...
            // Check for duplicates by ID (primary check for WatermelonDB)
            const recordId = filtered.id || filtered.ID;
            if (
              recordId &&
              existingRecords.some((existing: any) => {
                const existingId = existing.id || existing._raw?.id;
                return existingId?.toLowerCase() === recordId.toLowerCase();
              })
            ) {
              skippedCount++;
            ...
            // Import record - WatermelonDB repositories handle the database operations
            ...
            // Check if this is a duplicate key error from WatermelonDB
            if (errorMessage.includes("Duplicate key for property id")) {
              skippedCount++;
              const idMatch = errorMessage.match(/id: ([^\s]+)/);
              const duplicateId = idMatch ? idMatch[1] : "unknown";
              warnings.push(`Row ${rowNumber}: Duplicate key '${duplicateId}' skipped`);
            } else {
              failedCount++;
              errors.push({
                modelName: config.displayName,
