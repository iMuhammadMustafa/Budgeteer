...
import { RecurringType, AutoApplyAction, RECURRING_CONSTANTS } from "@/src/types/recurring";
...
export class AutoApplyEngine implements IAutoApplyEngine {
  ...
  async batchApplyTransactions(recurrings: Recurring[], tenantId: string, userId: string): Promise<BatchApplyResult> {
    ...
    for (let i = 0; i < recurrings.length; i += batchSize) {
      ...
      const batchPromises = batch.map(recurring => this.applyWithTimeout(recurring, tenantId, userId));
  ...
  private validateRecurringForExecution(recurring: Recurring): {
    isValid: boolean;
    ...
    if (!recurring.isactive) {
      errors.push(
        new RecurringValidationError("isactive", "required", recurring.isactive, "Recurring transaction is not active"),
      );
    }
  ...
  private async applyTransferTransaction(recurring: Recurring, tenantId: string, userId: string): Promise<string[]> {
    ...
    const transactions = await this.transactionRepo.createMultipleTransactions([
      primaryTransaction,
      transferTransaction,
    ]);
