...
class MockStorageProvider implements ForeignKeyDataProvider {
  ...
  async getRecords(table: string, filter?: any): Promise<any[]> {
    const tableMap = this.data.get(table);
    if (!tableMap) return [];
    let records = Array.from(tableMap.values());
    if (filter) {
      // Simple filter: match all key-value pairs in filter
...
describe("Schema Utilities Integration", () => {
  ...
  describe("Complete CRUD Validation Workflow", () => {
    it("should validate complete account creation workflow", async () => {
      ...
      const validationResult = await validateCRUDOperation("create", "accounts", newAccount, mockProvider);
    ...
    it("should validate complete transaction creation workflow", async () => {
      ...
      // Runtime validation
      const validationResult = await validateCRUDOperation("create", "transactions", newTransaction, mockProvider);
    ...
    it("should validate transfer transaction workflow", async () => {
      ...
      const validationResult = await validateCRUDOperation("create", "transactions", transferTransaction, mockProvider);
  ...
  describe("Error Detection and Handling", () => {
    it("should detect and report multiple validation errors", async () => {
      ...
      const validationResult = await validateCRUDOperation("create", "accounts", invalidAccount, mockProvider);
    ...
    it("should detect invalid transfer transaction", async () => {
      ...
      const validationResult = await validateCRUDOperation("create", "transactions", invalidTransfer, mockProvider);
    ...
    it("should detect missing transfer account for transfer transaction", async () => {
      ...
      const validationResult = await validateCRUDOperation("create", "transactions", incompleteTransfer, mockProvider);
  ...
  describe("Update Validation Workflow", () => {
    it("should validate account updates", async () => {
      ...
      const validationResult = await validateCRUDOperation("update", "accounts", updateData, mockProvider);
    ...
    it("should validate partial updates", async () => {
      ...
      const validationResult = await validateCRUDOperation("update", "accounts", partialUpdate, mockProvider);
    ...
    it("should detect invalid updates", async () => {
      ...
      const validationResult = await validateCRUDOperation("update", "accounts", invalidUpdate, mockProvider);
  ...
  describe("Delete Validation Workflow", () => {
    it("should validate deletion of existing record", async () => {
      const validationResult = await validateCRUDOperation("delete", "accounts", "acc1", mockProvider);

      expect(validationResult.isValid).toBe(true);
    });

    it("should detect deletion of non-existent record", async () => {
      const validationResult = await validateCRUDOperation("delete", "accounts", "nonexistent", mockProvider);
  ...
  describe("Complex Business Scenarios", () => {
    it("should handle recurring transaction validation", async () => {
      ...
      const validationResult = await validateCRUDOperation("create", "recurrings", recurringTransaction, mockProvider);
    ...
    it("should handle configuration validation", async () => {
      ...
      const validationResult = await validateCRUDOperation("create", "configurations", configuration, mockProvider);
  ...
  describe("Performance and Edge Cases", () => {
    it("should handle batch validation efficiently", async () => {
      ...
      const results = await runtimeValidator.validateBatch("accounts", accounts, "create", mockProvider);
    ...
    it("should handle null and undefined values correctly", async () => {
      ...
      const validationResult = await validateCRUDOperation("create", "accounts", accountWithNulls, mockProvider);
    ...
    it("should handle edge case amounts and dates", async () => {
      ...
      const validationResult = await validateCRUDOperation("create", "transactions", edgeCaseTransaction, mockProvider);
