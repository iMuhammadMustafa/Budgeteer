...
export class ErrorMapper {
  ...
  static mapError(error: any, context: ErrorMappingContext): StorageError {
    ...
    // If it's already a StorageError, return as-is but update context
  ...
  static extractErrorDetails(error: any): {
    ...
    const fieldMatch = message.match(/column "?(\w+)"?/i) ||
                      message.match(/field "?(\w+)"?/i) ||
                      message.match(/key "?(\w+)"?/i);
    if (fieldMatch) {
