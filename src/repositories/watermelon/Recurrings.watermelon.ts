...
import { Recurring as RecurringType } from "@/src/types/database/Tables.Types";
import { Account, Recurring, TransactionCategory } from "@/src/types/database/watermelon/models";
  ...
  implements IRecurringRepository
{
  ...
  override async findAll(tenantId: string, filters?: any): Promise<RecurringType[]> {
    ...
    for (const recurring of results as Recurring[]) {
      ...
      const accountQuery = await db
        ...
        .query(Q.where("id", recurring.sourceaccountid), Q.where("tenantid", tenantId), Q.where("isdeleted", false));
      (mappedRecurring as any).source_account = accountQuery[0] as Account | undefined;
      ...
      if (recurring.categoryid) {
        const categoryQuery = await db
          ...
          .query(Q.where("id", recurring.categoryid), Q.where("tenantid", tenantId), Q.where("isdeleted", false));
        category = categoryQuery[0] as TransactionCategory | undefined;
        (mappedRecurring as any).category = category;
      }
  ...
  override async findById(id: string, tenantId: string): Promise<RecurringType | null> {
    const db = await this.getDb();
    ...
    const model = query[0] as Recurring;
    ...
    const accountQuery = await db
      .get(TableNames.Accounts)
      .query(Q.where("id", model.sourceaccountid), Q.where("tenantid", tenantId), Q.where("isdeleted", false));
    (mappedRecurring as any).source_account = accountQuery[0] as Account | undefined;
