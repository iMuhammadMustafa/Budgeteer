...
export default function ChartLegend({
  ...
  return (
    <View className=" justify-center mt-2 px-2 pb-2">
          ...
          >
            <View
              style={{
                width: 10,
                height: 10,
                backgroundColor: index < colors.length ? colors[index] : "red",
                borderRadius: 2,
              }}
            />
            <ThemedText className="text-center">
              {verticalLegend
                ? `${item.x}: ${formatCurrency(item.y, false)} ${totalValue ? `(${((item.y / totalValue) * 100).toFixed(0)}%)` : ""}`
                : `${item.x}\n${formatCurrency(item.y, false)} ${totalValue ? `(${((item.y / totalValue) * 100).toFixed(0)}%)` : ""}`}
            </ThemedText>
          </View>
        ))}
      </ScrollView>
