...
function TitleBar({ width = 140 }: { width?: number }) {
  return <SkeletonBlock width={width} height={14} radius={6} style={{ alignSelf: "center", marginBottom: 8 }} />;
}
  ...
  return (
    <SkeletonCard>
      ...
      <View className="flex-row items-end justify-center gap-3" style={{ height: 130, paddingHorizontal: 16 }}>
        {barHeights.map((h, i) => (
          <View key={i} style={{ height: h, width: 28, backgroundColor: SKELETON_COLOR, borderRadius: 4 }} />
        ))}
      </View>
    ...
    <SkeletonCard>
      ...
      <View style={{ height: 130, width: 310, position: "relative", alignSelf: "center" }}>
        ...
        {[30, 60, 90].map((y, i) => (
          <View
            key={`grid-${i}`}
            style={{ position: "absolute", left: 0, right: 0, bottom: y, height: 1, backgroundColor: "#f0f0f0" }}
          />
        ))}
        {/* Dots */}
      ...
      <View className="flex-row items-center justify-center gap-3 mt-2 flex-wrap px-4">
        {[70, 55, 60, 50].map((w, i) => (
          <View key={i} className="flex-row items-center gap-1">
            ...
            <SkeletonBlock width={w} height={8} radius={4} />
          </View>
        ))}
      </View>
      <PeriodControlsPlaceholder />
      ...
      <View className="px-4" style={{ width: 280, alignSelf: "center" }}>
        ...
        <View className="flex-row justify-between mb-2">
          {["M", "T", "W", "T", "F", "S", "S"].map((_, i) => (
            <SkeletonBlock key={i} width={28} height={8} radius={4} />
          ))}
        </View>
        {/* 5 rows of day cells */}
