...
CREATE POLICY "Tenant access" ON AccountCategories FOR SELECT USING (TenantId = auth.tenantid());
...
CREATE POLICY "Tenant access" ON Accounts FOR SELECT USING (TenantId = auth.tenantid());
...
CREATE POLICY "Tenant access" ON TransactionGroups FOR SELECT USING (TenantId = auth.tenantid());
...
CREATE POLICY "Tenant access" ON TransactionCategories FOR SELECT USING (TenantId = auth.tenantid());
...
CREATE POLICY "Tenant access" ON Transactions FOR SELECT USING (TenantId = auth.tenantid());
